...
### Option B — Git URL (once pushed public)
...
Each expects the published `reify-server.dll` — build it from
`src/Server`:
...
- `REIFY_BRIDGE_PORT` — override the default `17777` (both Unity side
  and server side must agree)
...
animation clips, input system, + meta/reflection/batch.
